...
class TestAround<T : Any> : TestRunner<T> {
     ...
     * BeforeTwiceAndAfterTwiceTestClass(), BeforeAndAfterTestClass, AfterOnlyTestClass,
     * BeforeOnlyTestClass, EmptyTestClass
    ...
    private fun runAllAfter() =
        stepsMemberFunctions.filter { it.name.startsWith(AFTER) }.forEach {
             ...
             * Выдает исключение, если количество указанных аргументов не равно размеру параметров
             * или если их типы не совпадают с типами параметров.
             */
        }
     ...
     */
    private fun T.beforeAfter(beforeAfter: String) = this::class.declaredFunctions
        .filter { kFunction -> kFunction.name.contains(beforeAfter) }
